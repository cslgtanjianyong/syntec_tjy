...
namespace DAL
{
    ...
    public partial class Rule
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Model.Rule model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into Rule(");
            strSql.Append("reportSource,position,symbol,aim,planType,timeType,timeSpace,everydayFreSpace,startTime,endTime,startDate,endDate,email,article,attachmentSource,nextDoTimeDate)");
            ...
            parameters[2].Value = model.symbol;
            parameters[3].Value = model.aim;
            parameters[4].Value = model.planType;
            parameters[5].Value = model.timeType;
            parameters[6].Value = model.timeSpace;
            parameters[7].Value = model.everydayFreSpace;
            ...
            parameters[13].Value = model.article;
            parameters[14].Value = model.attachmentSource;
            parameters[15].Value = model.nextDoTimeDate;
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.Rule model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.Rule GetModel(int ID)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 ID,reportSource,position,symbol,aim,planType,timeType,timeSpace,everydayFreSpace,startTime,endTime,startDate,endDate,email,article,attachmentSource,nextDoTimeDate from Rule ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.Rule DataRowToModel(DataRow row)
        {
           Model.Rule model = new Model.Rule();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,reportSource,position,symbol,aim,planType,timeType,timeSpace,everydayFreSpace,startTime,endTime,startDate,endDate,email,article,attachmentSource,nextDoTimeDate ");
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" ID,reportSource,position,symbol,aim,planType,timeType,timeSpace,everydayFreSpace,startTime,endTime,startDate,endDate,email,article,attachmentSource,nextDoTimeDate ");
            strSql.Append(" FROM Rule ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
